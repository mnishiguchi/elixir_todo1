defmodule ElixirTodo.TodoCrud do
  ...
  """
  def add_entry(
        %TodoCrud{collection: collection, auto_id: auto_id} = instance,
        %{date: _date, title: _title} = entry
      ) do
    # Set the id for the entry being added.
    new_entry = entry |> Map.put(:id, auto_id)

    # Add the new entry to the collection and increment the `auto_id` field.
    updated_collection = collection |> Map.put(auto_id, new_entry)
  ...
  """
  def entries(
        %TodoCrud{collection: collection} = _instance,
        date
      ) do
  ...
  def update_entry(
        %TodoCrud{collection: collection} = instance,
        entry_id,
    ...
    case collection[entry_id] do
      ...
      entry ->
        ...
        updated_collection = collection |> Map.put(updated_entry.id, updated_entry)
        %TodoCrud{instance | collection: updated_collection}
    end
  end
